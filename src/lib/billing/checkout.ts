import { LIVE_TEST_CARD_MESSAGE } from "./credentials";
import { CHECKOUT_THEME } from "./plan";
import type { CheckoutSession } from "./types";
import { isPaymentAppHost, paymentOrigin } from "@/lib/public-origins";
...
function errorDescription(response: Record<string, unknown>, fallback: string): string {
  ...
  const body = error as { description?: unknown; reason?: unknown; step?: unknown };
  const reason = String(body.reason ?? "");
  const step = String(body.step ?? "");
  if (reason === "authentication_failed" || step === "payment_authentication") {
    return LIVE_TEST_CARD_MESSAGE;
  }
  if (typeof body.description === "string" && body.description.trim()) {
    if (/authentication failed/i.test(body.description)) return LIVE_TEST_CARD_MESSAGE;
...
export async function openRazorpayCheckout(session: CheckoutSession): Promise<CheckoutConfirmation | null> {
  ...
  return new Promise((resolve, reject) => {
    const checkout = new Razorpay({
      ...
      ...(isPaymentAppHost(window.location.hostname)
        ? { callback_url: `${paymentOrigin()}/` }
        : {}),
      handler: (response: {
